...
func SM4Encrypt(plainText, key []byte) ([]byte, error) {
	...
	blockMode := cipher.NewCBCEncrypter(block, sm4IV)
	cryted := make([]byte, len(origData))
	blockMode.CryptBlocks(cryted, origData)
	cryted = append(sm4IV, cryted...)
	return cryted, nil
}
...
func SM4Decrypt(cipherText, key []byte) ([]byte, error) {
	...
	sm4IV := cipherText[:16]
	blockMode := cipher.NewCBCDecrypter(block, sm4IV)
	origData := make([]byte, len(cipherText)-16)
	blockMode.CryptBlocks(origData, cipherText[16:])
...
func sm4CBCPassword(password []byte) []byte {
	if len(password) < 16 {
		pad := byte('@')
		password = crypto.RightPadding(password, 16, pad)
	} else {
...
func randomIV() []byte {
	...
	rand := append(IntToBytes(r.Intn(1<<63-1)), IntToBytes(r.Intn(1<<63-1))...)
	return rand
}
