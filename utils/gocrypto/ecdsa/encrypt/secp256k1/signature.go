...
// Sign use default S256 curve and compressed is false
func Sign(msg, priv []byte) ([]byte, error) {
	privKey, _ := PrivKeyFromBytes(S256(), priv)
	sig, err := SignCompact(S256(), privKey, msg, false)
	if err != nil {
		return nil, err
	}
	return sig, nil
...
func RecoverPubkey(msg []byte, sig []byte) ([]byte, error) {
	publicKey, isCompressed, err := RecoverCompact(S256(), sig, msg)
	if err != nil {
		return nil, err
	}
	...
	if isCompressed {
		serializeBytes = publicKey.SerializeCompressed()
	} else {
		serializeBytes = publicKey.SerializeUncompressed()
	}
...
func RecoverCompact(curve *KoblitzCurve, signature,
	...
	iteration := int((signature[64]) & ^byte(4))
	...
	return key, ((signature[64]) & 4) == 4, nil
}
